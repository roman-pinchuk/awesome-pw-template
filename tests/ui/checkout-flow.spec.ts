...
test.describe('SauceDemo checkout flow', () => {
  test('completes a full purchase journey', { tag: '@smoke', annotation: { type: 'feature', description: 'Checkout' } }, async ({
    ...
    await expect.configure({ message: 'Expected URL to navigate to checkout step one' })(page).toHaveURL(URLS.CHECKOUT_STEP_ONE);
    await checkoutStepOnePage.submitCheckout(DEFAULT_ADDRESS);

    await expect.configure({ message: 'Expected URL to navigate to checkout overview' })(page).toHaveURL(URLS.CHECKOUT_STEP_TWO);
    await expect.configure({ message: 'Expected item to appear in overview' })(checkoutStepTwoPage.cartItems).toHaveCount(1);

    await checkoutStepTwoPage.finish();
    await expect.configure({ message: 'Expected URL to navigate to checkout complete' })(page).toHaveURL(URLS.CHECKOUT_COMPLETE);
  ...
  test('shows error for empty checkout form', { annotation: { type: 'feature', description: 'Checkout' } }, async ({
    ...
    await expect.configure({ message: 'Expected error for missing checkout fields' })(checkoutStepOnePage.errorMessage).toBeVisible();
  });
});
