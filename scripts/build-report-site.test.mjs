...
test('builds the Pages site and prunes old Allure reports by count and size', async () => {
  ...
  try {
    ...
    await mkdir(join(source, 'ctrf'), { recursive: true });
    ...
    await writeFile(join(source, 'ctrf', 'index.html'), 'ctrf');
    for (const number of ['1', '2', '3']) {
      await mkdir(join(history, number), { recursive: true });
      await writeFile(join(history, number, 'index.html'), number.repeat(3000));
    }
    ...
    assert.equal(await readFile(join(site, 'ctrf', 'index.html'), 'utf8'), 'ctrf');
